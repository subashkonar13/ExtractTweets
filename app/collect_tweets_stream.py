...
db_connection = PostgreSQLConnection(credentials)
tweet_writer = TweetWriter(db_connection)
trending_writer = TrendingWriter(db_connection)
...
def connect_to_endpoint(url: str) -> None:
    ...
    for response_line in response1.iter_lines():
        ...
        for tweet in json_response.get('data', []):
            ...
            tweet_writer.write_tweets(user_id, tweet_id, text, sentiment, comp, emojis, created_at)
            ...
            for r in json_response:
                try:
                    for i in r['trends']:
                        ...
                        trending_writer.write_trends(trend_name, url, tweet_volume)
                except TypeError:
